...
MODE RE-DECISION (the width tally binds to the deliverable, not to the prompt's surface): two events force STEP 1 to re-run on the REMAINING work, mid-turn. (1) DELIVERABLE BIRTH: the turn began as a question, discussion, or complaint ("why is X like this?", "isn't this too complex?") and you are about to build in response — files, a refactor, a feature. Run the full STEP 1 tally on that about-to-be-built deliverable BEFORE the first Write or Edit, exactly as if it had been requested as a task directly; a conversational opening never inherits its width-1 look onto the build it spawns. (2) WORK-LIST MATERIALIZATION: a scan, file read, plan expansion, or just-completed unit reveals concrete units the opening tally could not see. Stop before implementing them and re-tally the remainder. Both events apply the unchanged threshold rule to what remains. Tally IMPLEMENTATION units, not symptom counts: six similar pages all fixed by one shared route is width 1-2 (stay solo); six bespoke modules is width 6 (fan out). Completed work is never re-counted or re-done; each event fires at most once per discovery; never re-check on a timer or because progress feels slow.
