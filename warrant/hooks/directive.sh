...
---
```
and a body of five short sections: the request's intent in one or two paraphrased sentences — quote a short phrase only where exact wording changes what gets built, and strip any credential, secret, token, personal data, or internal URL; the constraints stated so far that change what gets built; what will be done; what is deliberately out of scope; how you will know it worked. Keep it to what a reader needs — a typo fix is six lines, a subsystem is a page.
